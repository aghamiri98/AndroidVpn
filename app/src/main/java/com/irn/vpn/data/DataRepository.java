...
public class DataRepository {
    ...
    private MediatorLiveData<ContentResource> premiumContent3 = new MediatorLiveData<>();
    private MediatorLiveData<ContentResource> premiumContent4 = new MediatorLiveData<>();
 ...
 premiumContent3.addSource(webDataSource.getPremiumContent3(),
                new Observer<ContentResource>() {
                    @Override
                    public void onChanged(ContentResource contentResource) {
                        premiumContent3.postValue(contentResource);
                    }
                });
 premiumContent4.addSource(webDataSource.getPremiumContent4(),
                new Observer<ContentResource>() {
                    @Override
                    public void onChanged(ContentResource contentResource) {
                        premiumContent4.postValue(contentResource);
                    }
                });
    ...
    public MediatorLiveData<ContentResource> getPremiumContent3() { return premiumContent3; }
    public MediatorLiveData<ContentResource> getPremiumContent4() { return premiumContent4; }
    ...
    public void updateSubscriptionsFromNetwork(
        ...
        if (remoteSubscriptions != null) {
            ...
            boolean updatePremium3 = false;
            boolean updatePremium4 = false;
            for (SubscriptionStatus subscription : remoteSubscriptions) {
                if (Constants.BASIC_SKU.equals(subscription.sku)) {
                    updateBasic = true;
                } else if (Constants.PREMIUM_SKU.equals(subscription.sku)) {
                    // Premium subscribers get access to basic content as well.
                    updateBasic = true;
                    updatePremium = true;
                } else if (Constants.PREMIUM_SKU3.equals(subscription.sku)) {
                    // Premium subscribers get access to basic content as well.
                    updateBasic = true;
                    updatePremium = true;
                    updatePremium3 = true;
                } else if (Constants.PREMIUM_SKU4.equals(subscription.sku)) {
                    // Premium subscribers get access to basic content as well.
                    updateBasic = true;
                    updatePremium = true;
                    updatePremium3 = true;
            ...
            if (updatePremium3) {
                // Fetch the premium content.
                webDataSource.updatePremiumContent3();
            } else {
                // If we no longer own this content, clear it from the UI.
            ...
            if (updatePremium4) {
                // Fetch the premium content.
                webDataSource.updatePremiumContent4();
            } else {
                // If we no longer own this content, clear it from the UI.
