...
public class FakeServerFunctions implements ServerFunctions {
    ...
    private final MutableLiveData<ContentResource> premiumContent3 = new MutableLiveData<>();
    private final MutableLiveData<ContentResource> premiumContent4 = new MutableLiveData<>();
    ...
    public void updatePremiumContent3() {
        List<SubscriptionStatus> subs = subscriptions.getValue();
        if (subs == null || subs.isEmpty()) {
            premiumContent3.postValue(null);
            return;
        }
        if (BillingUtilities.isPremiumContent(subs.get(0))) {
            premiumContent3.postValue(new ContentResource("https://example.com/premium.jpg"));
        } else {
            premiumContent3.postValue(null);
        }
    }
    ...
    public void updatePremiumContent4() {
        List<SubscriptionStatus> subs = subscriptions.getValue();
        if (subs == null || subs.isEmpty()) {
            premiumContent4.postValue(null);
            return;
        }
        if (BillingUtilities.isPremiumContent(subs.get(0))) {
            premiumContent4.postValue(new ContentResource("https://example.com/premium.jpg"));
        } else {
            premiumContent4.postValue(null);
        }
    }
    ...
    public void registerSubscription(String sku, String purchaseToken) {
        ...
        switch (sku) {
            ...
            case Constants.PREMIUM_SKU3:
                subscriptions.postValue(Collections.singletonList(createFakePremiumSubscription3()));
                break;
            case Constants.PREMIUM_SKU4:
                subscriptions.postValue(Collections.singletonList(createFakePremiumSubscription4()));
                break;
            default:
    ...
    private SubscriptionStatus createFakePremiumSubscription3() {
        SubscriptionStatus subscription = new SubscriptionStatus();
        subscription.isEntitlementActive = true;
        subscription.willRenew = true;
        subscription.sku = Constants.PREMIUM_SKU3;
        subscription.isAccountHold = false;
        subscription.isGracePeriod = false;
        subscription.purchaseToken = null;
        subscription.subAlreadyOwned = false;
    ...
    private SubscriptionStatus createFakeAccountHoldSubscription3() {
        SubscriptionStatus subscription = new SubscriptionStatus();
        subscription.isEntitlementActive = false;
        subscription.willRenew = true;
        subscription.sku = Constants.PREMIUM_SKU3;
    ...
    private SubscriptionStatus createFakePremiumSubscription4() {
        SubscriptionStatus subscription = new SubscriptionStatus();
        subscription.isEntitlementActive = true;
        subscription.willRenew = true;
        subscription.sku = Constants.PREMIUM_SKU4;
        subscription.isAccountHold = false;
        subscription.isGracePeriod = false;
        subscription.purchaseToken = null;
        subscription.subAlreadyOwned = false;
    ...
    private SubscriptionStatus createFakeAccountHoldSubscription4() {
        SubscriptionStatus subscription = new SubscriptionStatus();
        subscription.isEntitlementActive = false;
        subscription.willRenew = true;
        subscription.sku = Constants.PREMIUM_SKU4;
    ...
    private SubscriptionStatus createFakeCanceledPremiumSubscription3() {
        SubscriptionStatus subscription = new SubscriptionStatus();
        subscription.isEntitlementActive = true;
        subscription.willRenew = false;
        subscription.sku = Constants.PREMIUM_SKU3;
    ...
    private SubscriptionStatus createFakeCanceledPremiumSubscription4() {
        SubscriptionStatus subscription = new SubscriptionStatus();
        subscription.isEntitlementActive = true;
        subscription.willRenew = false;
        subscription.sku = Constants.PREMIUM_SKU4;
